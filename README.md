...
Every financial fact, structural taxonomy, and logical rule is stored as a locally-addressable, human-readable Markdown file (`.md`) wrapped in rich JSON-LD/YAML metadata. The system bridges deterministic financial logic (SBRM/OIM) with a machine-to-machine micro-transactional economy via the Lightning Network.
...
## 5. The Standard Execution Pipeline
To process new data and generate a monetized SBRM proof, execute the pipeline in this strict sequence:
...
5. **Verify Mathematics:** `python evaluate_oim_report.py`
6. **Serve API:** Boot `python l402_middleware.py` in Terminal 1, and request proofs via `python client_relay_ping.py` in Terminal 2.

## 6. The L402 Commerce Bridge
LodgeiT Global utilizes the L402 Protocol to monetize heavy computational accounting proofs. Unauthenticated requests are rejected (`HTTP 402`) with a Lightning Invoice. Upon cryptographic settlement via Bitcoin Satoshis, the SWI-Prolog engine executes and returns the validated JSON-LD payload.
